...
class SagePayCw_Entity_ExternalCheckoutContext extends Customweb_Payment_ExternalCheckout_AbstractContext {
	...
	protected function loadPaymentMethodByMachineName($machineName){
		$paymentMethods = SagePayCw_Util::getPaymentMethods(true);
		foreach($paymentMethods as $method) {
			$instance = SagePayCw_Util::getPaymentMehtodInstance($method);
	 ...
	 * @return SagePayCw_Entity_ExternalCheckoutContext
	 */
	public static function getContextById($id, $loadFromCache = true) {
		return SagePayCw_Util::getEntityManager()->fetch('SagePayCw_Entity_ExternalCheckoutContext', $id, $loadFromCache);
	}
	...
	public static function getReusableContextFromCookie($loadFromCache = true) {
		if (isset($_COOKIE['sagepaycw-woocommerce-context-id']) && isset($_COOKIE['sagepaycw-woocommerce-context-key'])) {
			try {
				$result = SagePayCw_Util::getEntityManager()->searchByFilterName('SagePayCw_Entity_ExternalCheckoutContext', 'loadContextCookie', array('>contextId' => $_COOKIE['sagepaycw-woocommerce-context-id'], '>cookieKey' => $_COOKIE['sagepaycw-woocommerce-context-key']), $loadFromCache);
				if (count($result) > 0) {
					$context = current($result);
					if ($context instanceof SagePayCw_Entity_ExternalCheckoutContext && $context->getState() == Customweb_Payment_ExternalCheckout_IContext::STATE_PENDING) {
						return $context;
					}
				}
			}
	...
	public function updateFromCart($cart) {
		...
		if(SagePayCw_ConfigurationAdapter::getExternalCheckoutPlacement() == 'checkout') {
		...
		$this->setInvoiceItems(SagePayCw_CartUtil::getInoviceItemsFromCart($cart));
