 ...
 *
 *
 * @Entity(tableName = 'woocommerce_sagepaycw_transactions')
 * @Filter(name = 'loadByPostId', where = 'postId = >postId', orderBy = 'postId')
 */
class SagePayCw_Entity_Transaction extends Customweb_Payment_Entity_AbstractTransaction {
	...
	protected function updateOrderStatus(Customweb_Database_Entity_IManager $entityManager, $orderStatus, $orderStatusSettingKey){
		...
		$this->changeLanguage();		
		$GLOBALS['woocommerce_sagepaycw__status_change'] = true;
		$order = $this->getOrder();
		$paymentMethod = SagePayCw_Util::getPaymentMehtodInstance($this->getPaymentClass());
		if ($orderStatusSettingKey != 'status_authorized' || $paymentMethod->getPaymentMethodConfigurationValue('status_authorized') != 'use-default') {
			$order->update_status($orderStatus, __('Payment Notification', 'woocommerce_sagepaycw'));
		}
		//Restore the original language
		$this->restoreLanguage();
		unset($GLOBALS['woocommerce_sagepaycw__status_change']);
	}
	...
	protected function authorize(Customweb_Database_Entity_IManager $entityManager){
		if ($this->getTransactionObject()->isAuthorized()) {
			...
			//Switch language to the transaction language so emails are translated correctly
			$this->changeLanguage();
			delete_post_meta( $orderPostId, '_sagepaycw_pending_state' );
			...
			if ($orderStatus != 'use-default') {
				$GLOBALS['woocommerce_sagepaycw_complete_order_status'] = $orderStatus;
				add_filter('woocommerce_payment_complete_order_status', 'woocommerce_sagepaycw_complete_order_status', 10 , 3);
			}
	...
	protected function forceTransactionFailing(){
		...
		if(SagePayCw_Util::getAuthorizedTransactionByPostId($this->getPostId()) !== null){
			//Another Transaction has already sucessfuly authorized this transaction, do not mark the order as cancelled
			return;
		}
		$this->getOrder()->add_order_note(__('Error Message: ', 'woocommerce_sagepaycw') . $message);
		$orderId= $this->getOrder()->get_id();
		delete_post_meta( $orderId, '_sagepaycw_pending_state' );
		...
		if ($this->getAuthorizationType() != Customweb_Payment_Authorization_Recurring_IAdapter::AUTHORIZATION_METHOD_NAME && $this->getTransactionObject()->getTransactionContext()->createRecurringAlias()) {
			...
			add_action( 'woocommerce_email', 'woocommerce_sagepaycw_unhook_subscription_cancel' );
		}
	...
	private function changeLanguage(){
		...
		$GLOBALS['woo_sagepaycwAuthorizeLanguage'] = $this->getTransactionObject()->getTransactionContext()->getOrderContext()->getLanguage()->getIetfCode();
		add_filter('locale', 'woocommerce_sagepaycw_locale');
	}
	
	private function restoreLanguage(){
		
		do_action( 'wpml_switch_language', null);
		remove_filter('locale', 'woocommerce_sagepaycw_locale');
	}
