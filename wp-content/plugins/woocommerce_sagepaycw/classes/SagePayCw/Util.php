 ...
 *
 */
SagePayCw_Util::bootstrap();

require_once 'SagePayCw/Database/LinkAccessor.php';
require_once 'SagePayCw/EntityManager.php';
...
require_once 'SagePayCw/ContextCleanUpBean.php';
...
require_once 'SagePayCw/TransactionCleanUpBean.php';
require_once 'SagePayCw/Util.php';
require_once 'Customweb/Core/Url.php';
require_once 'SagePayCw/Entity/PaymentCustomerContext.php';
require_once 'SagePayCw/LayoutRenderer.php';
...
class SagePayCw_Util {
	 ...
	 * @return SagePayCw_PaymentMethod
	 */
	...
	public static function getPluginUrl($controller, array $params = array(), $action = null){
		...
		if (isset($_REQUEST['woo-sagepaycw-lang'])) {
			$params['woo-sagepaycw-lang'] = $_REQUEST['woo-sagepaycw-lang'];
		}		
		else if (defined('ICL_LANGUAGE_CODE')) {
			$params['woo-sagepaycw-lang'] = ICL_LANGUAGE_CODE;
		}
		else {
			$tmpLang = apply_filters( 'wpml_current_language', '' );
			if(!empty($tmpLang)){
				$params['woo-sagepaycw-lang'] = $tmpLang;
			}
		}
		...
		$urlString = get_permalink(get_option('woocommerce_sagepaycw_page'));
		if(isset($params['woo-sagepaycw-lang'])){
			$urlString = apply_filters('wpml_permalink', $urlString, $params['woo-sagepaycw-lang']);
		}
		...
		return apply_filters('woocommerce_sagepaycw_plugin_url', $complete, $url->getBaseUrl() . $url->getPath(), 
				$url->getQueryAsArray());
	}
	...
	public static function getPermalinkIdModified($id){
		...
		if (isset($_REQUEST['woo-sagepaycw-lang'])) {
			$language = $_REQUEST['woo-sagepaycw-lang'];
		}
		else if (defined('ICL_LANGUAGE_CODE')) {
	...
	private static function checkRequirements() {
		...
		if (!is_plugin_active('woocommerce/woocommerce.php')){
			$errors[] = sprintf(__("Woocommerce has to be active.", "woocommerce_sagepaycw"));
		}
		else{
			...
			if (version_compare ($woocommerce_data['Version'] , '3.0.0', '<')){
				$errors[] = sprintf(__("Woocommerce %s+ is required. (You're running version %s)", "woocommerce_sagepaycw"), '3.0.0', $woocommerce_data['Version']);
			}
		}		
		if(!empty($errors)){
			$title = __('Could not activate plugin WooCommerce SagePayCw', 'woocommerce_sagepaycw');
			$message = '<h1><strong>'.$title.'</strong></h1><br/>'.
					'<h3>'.__('Please check the following requirements before activating:', 'woocommerce_sagepaycw').'</h3>'.
					'<ul><li>'.
	...
	public static function installPlugin(){
		...
		$manager = new Customweb_Database_Migration_Manager(self::getDriver(), dirname(__FILE__) . '/Migration/', 
				$wpdb->prefix . 'woocommerce_sagepaycw_schema_version');
		$manager->migrate();
		
		//Create Page
		$optionValue = get_option('woocommerce_sagepaycw_page');
		$pageContent = '[woocommerce_sagepaycw]';
		$pageSlug = 'woo_sagepaycw';
		$pageTitle = 'Opayo Checkout';
		...
		update_option('woocommerce_sagepaycw_page', $pageId);
	}
	...
	public static function includeTemplateFile($templateName, $variables = array()){
		...
		$templateName = 'sagepaycw_' . $templateName;
	...
	public static function createContainer(){
		if (self::$container === null) {
			...
			// @formatter:off
			$storage = new Customweb_Storage_Backend_Database(self::getEntityManager(), self::getDriver(), 'SagePayCw_Entity_Storage');
			$provider->addObject(SagePayCw_ContextRequest::getInstance())
				->addObject(self::getEntityManager())
				->addObject(self::getDriver())
				->addObject(new SagePayCw_LayoutRenderer())
				->addObject(new Customweb_Cache_Backend_Memory())
				->add('databaseTransactionClassName', 'SagePayCw_Entity_Transaction')
				->addObject(self::getAssetResolver())
				->addObject($storage)
				->addObject(new SagePayCw_ContextCleanUpBean(self::getEntityManager()))
				->addObject(new SagePayCw_TransactionCleanUpBean(self::getEntityManager()));
			// @formatter:om
	...
	public static function getEntityManager(){
		if (self::$entityManager === null) {
			$cache = new Customweb_Cache_Backend_Memory();
			self::$entityManager = new SagePayCw_EntityManager(self::getDriver(), $cache);
		}
	...
	public static function getAssetResolver(){
		$simple = array();
		$simple[] = new Customweb_Asset_Resolver_Simple(get_template_directory(). '/woocommerce_sagepaycw/', null,
				array(
					'application/x-phtml'
				));
		...
		$simple[] = new Customweb_Asset_Resolver_Simple(get_template_directory(). '/woocommerce_sagepaycw/', get_stylesheet_directory_uri() . '/woocommerce_sagepaycw/');
	...
	public static function getDriver(){
		if (self::$driver === null) {
			global $wpdb;
			$link = SagePayCw_Database_LinkAccessor::getSagePayCwLink($wpdb);
	...
	public static function getPaymentCustomerContext($customerId){
		...
		if (!isset(self::$paymentCustomerContexts[$customerId])) {
			$entities = self::getEntityManager()->searchByFilterName('SagePayCw_Entity_PaymentCustomerContext', 'loadByCustomerId', 
					array(
			...
			if (count($entities) > 0) {
				...
				$context = new SagePayCw_Entity_PaymentCustomerContext();
	...
	public static function persistPaymentCustomerContext(Customweb_Payment_Authorization_IPaymentCustomerContext $context){
		if ($context instanceof SagePayCw_Entity_PaymentCustomerContext) {
	...
	public static function thankYouPageHtml($orderId){
		...
		if (!empty($paymentInformation)) {
			echo '<div class="woocommerce_sagepaycw-payment-information" id="woocommerce_sagepaycw-payment-information">';
			echo "<h2>" . __('Payment Information', 'woocommerce_sagepaycw') . "</h2>";
	...
	public static function orderEmailHtml($order, $sent_to_admin, $plain_text = false){		
		...
		$paymentInformation = trim($transactionObject->getPaymentInformation());		
		if(!empty($paymentInformation)) {
			echo '<div class="woocommerce_sagepaycw-email-payment-information" id="woocommerce_sagepaycw-email-payment-information">';
			echo "<h2>" . __('Payment Information', 'woocommerce_sagepaycw') . "</h2>";
			echo $paymentInformation;
			echo '</div>';
		}
	 ...
	 * @return SagePayCw_Entity_Transaction The matching transactions for the given transaction id
	 */
	public static function getTransactionById($id, $cache = true){
		return self::getEntityManager()->fetch('SagePayCw_Entity_Transaction', $id, $cache);
	}
	 ...
	 * @param boolean $cache load from cache
	 * @return SagePayCw_Entity_Transaction The matching transactions for the given transactionNumber
	 */
	public static function getTransactionByTransactionNumber($number, $cache = true){
		$transactions = self::getEntityManager()->searchByFilterName('SagePayCw_Entity_Transaction', 'loadByExternalId', 
				array(
	 ...
	 * @param boolean $cache load from cache
	 * @return SagePayCw_Entity_Transaction[] The matching transactions for the given order id
	 */
	public static function getTransactionsByOrderId($orderId, $cache = true){
		self::getPaymentMethods(true);
		return self::getEntityManager()->searchByFilterName('SagePayCw_Entity_Transaction', 'loadByOrderId', 
				array(
					'>orderId' => $orderId 
				), $cache);
	}
	...
	public static function getTransactionsByPostId($postId, $cache = true){
		self::getPaymentMethods(true);
		return self::getEntityManager()->searchByFilterName('SagePayCw_Entity_Transaction', 'loadByPostId',
				array(
					'>postId' => $postId
				), $cache);
	}
	...
	public static function getAliasTransactions($userId, $paymentMethod){
		...
		$aliases = array();
		$entities = self::getEntityManager()->search('SagePayCw_Entity_Transaction', 
	...
	public static function computeTransactionValidateHash(SagePayCw_Entity_Transaction $transaction) {
	...
	public static function tryToParseDate($date_string){
		$date_of_birth = false;
		$custom_date_of_birth_format = apply_filters('woocommerce_sagepaycw_custom_date_of_birth_format', '');
