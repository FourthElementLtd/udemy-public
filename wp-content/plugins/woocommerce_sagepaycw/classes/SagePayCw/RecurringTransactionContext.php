...
class SagePayCw_RecurringTransactionContext extends SagePayCw_TransactionContext implements Customweb_Payment_Authorization_Recurring_ITransactionContext
{
	private $initialTransactionId;
	
	private $initialTransaction;
	
	public function __construct(SagePayCw_Entity_Transaction $transaction, SagePayCw_RecurringOrderContext $orderContext) {
		parent::__construct($transaction, $orderContext);
		$initialTransactionId = $orderContext->getInitialTransactionId();
		$initialTransaction = SagePayCw_Util::getTransactionById($initialTransactionId);
	...
	public function getInitialTransaction() {
		if ($this->initialTransaction === null) {
			$this->initialTransaction = SagePayCw_Util::getTransactionById($this->initialTransactionId);
		}
