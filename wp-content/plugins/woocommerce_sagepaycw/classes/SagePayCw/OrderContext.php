...
class SagePayCw_OrderContext extends Customweb_Payment_Authorization_OrderContext_AbstractDeprecated {
	...
	public function __construct($order, Customweb_Payment_Authorization_IPaymentMethod $paymentMethod){
		...
		if ($sessionHandler != null) {
			$checkoutId = $sessionHandler->get('SagePayCwCheckoutId', null);
			if ($checkoutId === null) {
				$checkoutId = Customweb_Core_Util_Rand::getUuid();
				$sessionHandler->set('SagePayCwCheckoutId', $checkoutId);
			}
		}
		else {
		...
		foreach ($wooCommerceItems as $wooItem) {
			...
			if (Customweb_Util_Currency::compareAmount($discountAmount, 0, $this->getCurrencyCode()) > 0) {
				$discountItem = new Customweb_Payment_Authorization_DefaultInvoiceItem($sku . '-discount',
						__("Discount", "woocommerce_sagepaycw") . ' ' . $wooItem['name'], $taxRate, $discountAmount, $quantity,
	...
	public function getBillingState(){
		...
		return SagePayCw_Util::cleanUpStateField($state, $this->getBillingCountryIsoCode());
	...
	public function getBillingDateOfBirth(){
		...
		if (!empty($dateOfBirthString)) {
			$dateOfBirth = SagePayCw_Util::tryToParseDate($dateOfBirthString);
	...
	public function getShippingState(){
		...
		if (!empty($state)) {
			return SagePayCw_Util::cleanUpStateField($state, $this->getShippingCountryIsoCode());
		}
	...
	public function getOrderNumber(){
		$orderNumber = null;
		if (SagePayCw_ConfigurationAdapter::getOrderNumberIdentifier() == 'ordernumber') {
			$orderNumber = preg_replace('/[^a-zA-Z\d]/', '', $this->order->get_order_number());
		}
		else {
			$orderNumber = $this->order->get_id();
		}
		$existing = SagePayCw_Util::getTransactionsByOrderId($orderNumber);
		if (!empty($existing)) {
			$count = 0;
			while(true){
