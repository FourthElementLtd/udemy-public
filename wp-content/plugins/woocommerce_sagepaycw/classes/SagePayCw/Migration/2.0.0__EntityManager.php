...
class SagePayCw_Migration_2_0_0 implements Customweb_Database_Migration_IScript {

	public function execute(Customweb_Database_IDriver $driver){
		global $wpdb;
		
		$tableName = $wpdb->prefix . 'sagepaycw_transactions';
		...
		$tableNameNew = $entityManager->getTableNameForEntityByClassName('SagePayCw_Entity_Transaction');
		
		$driver->query("RENAME TABLE `" . $tableName . "` TO `" . $tableNameNew . "`")->execute();
		
		$driver->query(
				"CREATE TABLE IF NOT EXISTS `" . $wpdb->prefix . "woocommerce_sagepaycw_cc` ( 
