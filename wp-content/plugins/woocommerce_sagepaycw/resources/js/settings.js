...
jQuery(document).ready(function() {
	...
	jQuery('.sagepaycw-transaction-table .sagepaycw-more-details-button').each(function() {
		jQuery(this).click(function() {
			...
			var transactionId = mainRow.attr('id').replace('sagepaycw-main_row_', '');
...
(function($) {
	...
	var SagePayCwLineItemGrid = {
		decimalPlaces: 2,
		currencyCode: 'EUR',
		
		init: function() {
			this.decimalPlaces = parseFloat($("#sagepaycw-decimal-places").val());
			this.currencyCode = $("#sagepaycw-currency-code").val();
			this.attachListeners();
		},
		
		attachListeners: function() {
			$(".sagepaycw-line-item-grid input.line-item-quantity").each(function() {
				SagePayCwLineItemGrid.attachListener(this);
			});
			$(".sagepaycw-line-item-grid input.line-item-price-excluding").each(function() {
				SagePayCwLineItemGrid.attachListener(this);
			});
			$(".sagepaycw-line-item-grid input.line-item-price-including").each(function() {
				SagePayCwLineItemGrid.attachListener(this);
			});
		},
		...
		attachListener: function(element) {
			$(element).change(function() {
				SagePayCwLineItemGrid.recalculate(this);
			});
		...
		recalculate: function(eventElement) {
			...
			var row = $('.sagepaycw-line-item-grid tr[data-line-item-index="' + lineItemIndex + '"]');
