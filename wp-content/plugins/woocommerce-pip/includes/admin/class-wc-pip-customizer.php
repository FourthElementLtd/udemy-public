...
class WC_PIP_Customizer {
	...
	public function __construct() {
		...
		if ( isset( $_GET[ $this->customizer_trigger ] ) ) {
			...
			add_action( 'wp_enqueue_scripts', array( $this, 'dequeue_styles'  ), 1000 );
	 ...
	 *
	 * TODO remove this method either by version 3.5.0 or as part of WC 3.2 compatibility release {FN 2017-06-27}
	 *
	 * @internal
	 * @deprecated
	 *
	...
	public function dequeue_theme_styles() {
		_deprecated_function( 'WC_PIP_Customizer::dequeue_theme_styles()', '3.2.0', 'WC_PIP_Customizer::dequeue_styles()' );
		$this->dequeue_styles();
	}
	...
	public function dequeue_styles() {
		...
		if ( is_object( $wp_styles ) && isset( $_GET[ $this->customizer_trigger ] ) ) {
			...
			foreach ( $wp_styles->registered as $registered ) {
				...
				if ( is_object( $registered ) && ( SV_WC_Helper::str_ends_with( get_template_directory_uri() . '/style.css', $registered->src ) || SV_WC_Helper::str_ends_with( get_stylesheet_uri(), $registered->src ) ) ) {
	...
	public function dequeue_scripts() {
		...
		if ( is_object( $wp_scripts ) && isset( $_GET[ $this->customizer_trigger ] ) ) {
			...
			foreach ( $wp_scripts->registered as $registered ) {
				...
				if ( is_object( $registered ) && ( SV_WC_Helper::str_starts_with( $registered->src, $theme_url ) || ( SV_WC_Helper::str_starts_with( $registered->src, $plugins_url ) && ! SV_WC_Helper::str_starts_with( $registered->src, $plugins_url . 'woocommerce-pip/' ) ) ) ) {
