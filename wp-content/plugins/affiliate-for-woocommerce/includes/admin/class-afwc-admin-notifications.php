		...
		public function afwc_footer_text( $afw_rating_text ) {
			...
			if ( ! empty( $pagenow ) ) {
				...
				if ( in_array( $get_page, $afw_pages, true ) || 'affiliate-for-woocommerce-settings' === $get_tab ) {
					...
					$afw_rating_text = wp_kses_post( sprintf( __( 'If you like %1$sAffiliate For WooCommerce%2$s, please give us %3$s. A huge thanks from WooCommerce & StoreApps in advance!', 'affiliate-for-woocommerce' ), '<strong>', '</strong>', '<a target="_blank" href="' . esc_url( 'https://woocommerce.com/products/affiliate-for-woocommerce/#comments' ) . '" style="color: #96588a;">5-star rating</a>' ) );
				}
			}
		...
		public function afwc_update_footer_text( $afw_text ) {
			...
			if ( ! empty( $pagenow ) ) {
				...
				if ( in_array( $get_page, $afw_pages, true ) || 'affiliate-for-woocommerce-settings' === $get_tab ) {
					...
					$afw_text = sprintf( __( 'Have a feature request? Submit your request from our %s.', 'affiliate-for-woocommerce' ), '<a href="' . esc_url( 'https://ideas.woocommerce.com/forums/133476-woocommerce?category_id=379618' ) . '" target="_blank" style="color: #96588a;">idea board</a>' );
				}
			}
